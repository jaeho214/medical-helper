...
public interface PharmacyRepository extends JpaRepository<Pharmacy, Long> {
    ...
    List<Pharmacy> findByName(String name, String xPos, String yPos, int pageNum);


    @Query(nativeQuery = true, value = "select *, (6371*acos(cos(radians(:yPos))*cos(radians(p.y_pos))*cos(radians(p.x_pos)-radians(:xPos)) \n" +
            "+sin(radians(:yPos))*sin(radians(p.y_pos)))) AS distance from pharmacy p having distance < 7 order by distance asc limit :pageNum, 10;")
    List<Pharmacy> findOrderByDistance(String xPos, String yPos, int pageNum);
